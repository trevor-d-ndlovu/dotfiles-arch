echo -e "Offer new Akatsuki hotkeys\n"
...
if gum confirm "Switch to new hotkeys?"; then
  ...
  sed -i 's|source = ~/.local/share/akatsuki/default/hypr/bindings/tiling\.conf|source = ~/.local/share/akatsuki/default/hypr/bindings/clipboard.conf\
source = ~/.local/share/akatsuki/default/hypr/bindings/tiling-v2.conf|' ~/.config/hypr/hyprland.conf
fi
