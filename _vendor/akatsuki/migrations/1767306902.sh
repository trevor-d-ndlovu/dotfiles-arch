...
THEMES_DIR="$HOME/.config/akatsuki/themes"
CURRENT_THEME_LINK="$HOME/.config/akatsuki/current/theme"
...
elif [[ -f $HOME/.config/akatsuki/current/theme.name ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/akatsuki/current/theme.name")
fi

# Remove all symlinks from ~/.config/akatsuki/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  akatsuki-theme-set "$CURRENT_THEME_NAME"
else
  # Backup to ensure a theme is set if we can't deduce the name
  akatsuki-theme-set "Tokyo Night"
fi
