...
# Users can still opt-in to Cloudflare DNS using: akatsuki-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-akatsuki-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-akatsuki-dns.network
  ...
  echo "To use Cloudflare DNS, run: akatsuki-setup-dns Cloudflare"
fi